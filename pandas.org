...
**** Read and write data
#+begin_src hy :session Hy :results output
  (setv data (pd.read-csv "./pydata-book/examples/ex5.csv"))
  (print data)
  (data.to-csv "/tmp/out.csv")
#+end_src
...
**** Check that it's actually written
#+begin_src shell
cat /tmp/out.csv
#+end_src
...
**** Write to stdout with custom separator
#+begin_src hy :session Hy :results output
(data.to-csv sys.stdout :sep "|")
#+end_src
...
#+begin_src hy :session Hy :results output
(data.to-csv sys.stdout :na-rep "NULL")
#+end_src
...
**** Write ~Series~
#+begin_src hy :session Hy :results output
  (setv dates (pd.date-range "1/1/2000" :periods 7))
  (setv ts (pd.Series (np.arange 7) :index dates))
  (ts.to-csv sys.stdout :header False)
#+end_src
...
#+begin_src hy :session Hy :results output
  (import csv)
  (setv f (open "./pydata-book/examples/ex7.csv"))
...
**** Wrangle it
#+begin_src hy :session Hy :results output
  (with [f (open "./pydata-book/examples/ex7.csv")]
    (setv lines (list (csv.reader f))))

  (setv [header values] [(get lines 0) (cut lines 1)])
  (setx data-dict (dfor [v h] (zip header (zip #* values)) [v h]))
#+end_src
...
#+begin_src hy :session Hy :results silent
  ...
  (setv reader (csv.reader f :dialect my-dialect))
  (with [f (open "/tmp/mydata.csv" "w")]
    (setv writer (csv.writer f :dialect my-dialect))
...
**** Serialize JSON
#+begin_src hy :session Hy :results output
(setv asjson (json.dumps result))
#+end_src

#+RESULTS:
**** Converting JSON to DataFrame
#+begin_src hy :session Hy :results output
(setx siblings (pd.DataFrame (get result "siblings") :columns ["name" "age"]))
#+end_src
...
#+begin_src hy :session Hy :results output
  (setx data (pd.read_json "./pydata-book/examples/example.json"))
#+end_src
...
#+begin_src hy :session Hy :results output
  (print (.to-json data :orient "records"))
#+end_src
...
** Test
#+begin_src hy :session Hy :results file link :file "images/myfig.png"
...
#+begin_src hy :session Hy :results file link :file "images/subplots.png"
  (plt.clf)
  (setv fig (plt.figure))
  (setv ax1 (fig.add-subplot 2 2 1))
  (setv ax2 (fig.add-subplot 2 2 2))
  (setv ax3 (fig.add-subplot 2 2 3))
  (ax1.hist (np.random.randn 100) :bins 20 :color "k" :alpha 0.3)
  (ax2.scatter (np.arange 30)
               (+ (np.arange 30) (* 3 (np.random.randn 30))))
  (ax3.plot (.cumsum (np.random.randn 50)) "k--")
  (plt.grid)
  (plt.savefig "images/subplots.png" :dpi 140)
#+end_src
